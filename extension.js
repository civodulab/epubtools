...
function activate(context) {
  ...
  // disposable = vscode.commands.registerCommand("extension.epubMediaOverlay", function () {
  //   const mediaOverlays=require('./src/mediaoverlay')
  //   if (!util.testOEBPS()) {
  //     mesMessages.mesErreurs.erreurPathOEBPS();
  //     return; // No open text editor
  //   }
  ...
  //   mediaOverlays.epubMetadataMediaOverlay(d.fileName);
  ...
  disposable = vscode.commands.registerCommand("extension.epubError", function () {
    ...
    mesErreurs[1].length > 0 &&
      outputChannel.appendLine("- " + mesMessages.txtLangue["outputChannelTableauTh"]);
    mesErreurs[1].forEach((erreur) => {
      outputChannel.appendLine("\t" + erreur + "\n");
    });
    mesErreurs[0].length > 0 &&
      outputChannel.appendLine("- " + mesMessages.txtLangue["outputChannelTableauScope"]);
    ...
    if (outSpine) {
      outputChannel.appendLine(
        "- " +
          mesMessages.txtLangue["outputChannelPbSpine2"] +
          "(" +
          monOpf.toString() +
          ")"
      );
